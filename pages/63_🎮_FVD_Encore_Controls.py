...
st.set_page_config(
    page_title="🎮 FVD Encore Controls",
    page_icon="🎮",
    layout="wide",
...
if amr_missions_data:
    ...
    df = df[['amr_id', 'goal', 'enqueue_time', 'status']]
...
def execute_amr_missions(selected_rick_goal, selected_morty_goal):
    ...
    # If no running missions, enqueue new missions for RICK and MORTY
    response_rick = create_new_amr_mission(AMR.RICK.value, selected_rick_goal)
    response_morty = create_new_amr_mission(AMR.MORTY.value, selected_morty_goal)

    # You can handle the responses as needed, for example:
    st.success(f"New mission for RICK created with response: {response_rick.json()}")
    st.success(f"New mission for MORTY created with response: {response_morty.json()}")


st.markdown("## RICK and MORTY's controls.")
...
with col1:
    ...
    selected_rick_goal = goal_options[selected_option]
with col2:
    amr = AMR.MORTY
    selected_option = st.selectbox(f"Select where you would like to command {amr.name} to go.",
                 options=goal_options.keys(), index=list(goal_options.keys()).index("STAY_WHERE_IT_IS"))
    selected_morty_goal = goal_options[selected_option]
if st.button("Send commands", key="send_fleet_commands", use_container_width=True):
    execute_amr_missions(WorkCell(selected_rick_goal), WorkCell(selected_morty_goal))
