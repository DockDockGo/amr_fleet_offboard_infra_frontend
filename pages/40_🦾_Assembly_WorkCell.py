...
def get_enqueued_assembly_station_tasks(workcell, only_fetch_first_enqueued=True):
...
def fetch_tasks():
    task = get_enqueued_assembly_station_tasks(WorkCell.ASSEMBLY_STATION)
    if task is not None:
...
def complete_task():
    ...
    if st.session_state.current_task_type == TestbedTaskType.UNLOADING.value:
        create_testbed_task(WorkCell.ASSEMBLY_STATION, TestbedTaskType.PROCESSING, st.session_state.current_assembly_type_id)
    if st.session_state.current_task_type == TestbedTaskType.PROCESSING.value:
        create_testbed_task(WorkCell.ASSEMBLY_STATION, TestbedTaskType.LOADING, st.session_state.current_assembly_type_id)
...
if st.session_state.current_task_type is not None:
    if st.session_state.current_task_type == TestbedTaskType.UNLOADING.value:
        ...
        st.markdown("#### Please follow the instructions below to assemble the model using parts from the supplied kit.")
        pattern = f"{assembly_type_id_to_dir[st.session_state.current_assembly_type_id]}*/**/*.png"
        steps = sorted(glob.glob(pattern, recursive=True))
        for step in steps:
            st.image(step)
        if st.button("I've built the model!", use_container_width=True):
            complete_task()
