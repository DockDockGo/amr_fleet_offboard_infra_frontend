...
def get_enqueued_kitting_station_tasks(workcell, only_fetch_first_enqueued=True):
...
def fetch_tasks():
    task = get_enqueued_kitting_station_tasks(WorkCell.KITTING_STATION)
    if task is not None:
...
def complete_task():
    ...
    if st.session_state.current_task_type == TestbedTaskType.UNLOADING.value:
        create_testbed_task(WorkCell.KITTING_STATION, TestbedTaskType.PROCESSING, st.session_state.current_assembly_type_id)
    if st.session_state.current_task_type == TestbedTaskType.PROCESSING.value:
        create_testbed_task(WorkCell.KITTING_STATION, TestbedTaskType.LOADING, st.session_state.current_assembly_type_id)
...
if st.session_state.current_task_type is not None:
    if st.session_state.current_task_type == TestbedTaskType.UNLOADING.value:
        ...
        st.markdown("#### Please compile a kit with the following types and quantities of parts from the supplied bins:")
        st.image(os.path.join(assembly_type_id_to_dir[st.session_state.current_assembly_type_id], '1_1x.png'))
        if st.button("I've compiled the kit!", use_container_width=True):
            complete_task()
